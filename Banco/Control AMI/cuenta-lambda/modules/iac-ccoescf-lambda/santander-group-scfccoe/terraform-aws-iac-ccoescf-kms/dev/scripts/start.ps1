...
[string]$script:TFPlanRootPath = Join-Path -Path "$script:RootPath" -ChildPath "/../out"
...
$TFPlanRootPath = ($script:TFPlanRootPath | Resolve-Path).Path
...
try {
    ...
    if (!$?) {
        Invoke-Expression $script:SetProxyCommand
    ...
    [PSCustomObject]$script:PlanModuleCalls = $Plan.configuration.root_module.module_calls
    [string[]]$script:AllModules = $PlanModuleCalls | Get-Member -MemberType NoteProperty | Select-Object -ExpandProperty Name
    Write-Host "Modules FOUND:"
    $AllModules

    [System.Collections.ArrayList]$script:PlanModules = [System.Collections.ArrayList]@()

    foreach ($moduleName in $AllModules) {
        [PSCustomObject]$script:moduleCall = $PlanModuleCalls | Select-Object -ExpandProperty $moduleName
        [string]$script:moduleSource = $moduleCall.source        
        if ($moduleSource -ne "../") {            
            continue
        }
        $PlanModules.Add($moduleName)
    }    
    ...
    Write-Host
    [ushort]$script:SelectedModuleIndex = Read-Host "Enter the module index with changes to be performed. (Empty to perform all changes)"
    ...
    if ($script:CanApply -notlike "yes") {        
