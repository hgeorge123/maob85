...
locals {
  ...
  type_C_tags_not_dependency_with_dependencies_definition = {
    # [STAGE 3.0] REPLACE `default_value_definition` and `scope_definition` tokenized arguments.
    for tag_name, tag_definition in local.tags_definition_input_not_dependency_with_dependencies :
    tag_name => merge(
          ...
          for default_value_definition in tag_definition.default_value_definition :
          {
              ...
              "${condition_operand}" => [
                for condition_key, condition_values in condition_block : {
                  condition_key = (
                    ...
                    ? try(
                      # LOOK FOR `condition_key` as string interpolation in `local.type_B_tags_dependency_with_dependencies_value_resolver`.
                      lookup(local.type_B_tags_dependency_with_dependencies_value_resolver, condition_key).value,
                  ...
                  condition_values = [
                    for condition_value in condition_values : (
                      ...
                      ? try(
                        # LOOK FOR `condition_value` as string interpolation in `local.type_B_tags_dependency_with_dependencies_value_resolver`.
                        lookup(local.type_B_tags_dependency_with_dependencies_value_resolver, condition_value).value,
                        # IF no interpolation replacement is found, return null.
                        null
                      )
            ...
            format = [
              for format_argument in default_value_definition.format : (
                ...
                ? try(
                  # LOOK FOR `format_argument` as string interpolation in `local.type_B_tags_dependency_with_dependencies_value_resolver`.
                  lookup(local.type_B_tags_dependency_with_dependencies_value_resolver, format_argument).value,
                  # IF no interpolation replacement is found, return null.
                  null
                )
        ...
        scope = [ # BUILD `scope` with argument replacements (either scope_definition key or scope_definition values).
          for scope_key, scope_values in tag_definition.scope_definition : {
            condition_key = (
              ...
              ? try(
                # LOOK FOR `scope_definition` key as string interpolation in `local.type_B_tags_dependency_with_dependencies_value_resolver`.
                lookup(local.type_B_tags_dependency_with_dependencies_value_resolver, scope_key).value,
                # IF no interpolation replacement is found, return null.
                null
              )
            ...
            condition_values = [
              for scope_value in scope_values : (
                ...
                ? try(
                  # LOOK FOR `scope_definition` value as string interpolation in `local.type_B_tags_dependency_with_dependencies_value_resolver`.
                  lookup(local.type_B_tags_dependency_with_dependencies_value_resolver, scope_value).value,
                  # IF no interpolation replacement is found, return null.
                  null
                )
  ...
  type_C_tags_not_dependency_with_dependencies_default_value_fulfilled_conditions = {
    # [STAGE 3.1] EVALUATE all `default_value` conditions that are fulfilled in current context.
    for tag_name, tag_definition in local.type_C_tags_not_dependency_with_dependencies_definition :
    tag_name => {
  ...
  type_C_tags_not_dependency_with_dependencies_default_value_definition = {
    # [STAGE 3.2] TAKE only the first default_value with fulfilled condition (if any) according to the same order of precedence as they where defined in tag definition.
    for tag_name, fulfilled_default_value_definitions in local.type_C_tags_not_dependency_with_dependencies_default_value_fulfilled_conditions :
    tag_name => lookup(fulfilled_default_value_definitions, try(min(keys(fulfilled_default_value_definitions)...), ""), null)
  }

  type_C_tags_not_dependency_with_dependencies_default = tomap(
    {
      # [STAGE 3.3] BUILD default value for Type C tags.
      # If the tag definition contains a `default_value` format for current conditions, apply it to generate default value for tag.
      for tag_name, tag_default_value_definition in local.type_C_tags_not_dependency_with_dependencies_default_value_definition :
      tag_name => try(
  ...
  type_C_tags_not_dependency_with_dependencies_value_resolver = tomap(
    # Merge all interpolations (for locals, input vars and mandatory tags) in a single map.
    merge(
      local.type_B_tags_dependency_with_dependencies_value_resolver,
      {
        # [STAGE 3.4] Resolve value from given input or default for Type C tags.
        for tag_name in keys(local.type_C_tags_not_dependency_with_dependencies_definition) :
        "$${var.mandatory_tags.${tag_name}}" => {
          ...
          value = try(
            ...
            coalesce(
              ...
              lookup(local.type_C_tags_not_dependency_with_dependencies_default, tag_name, null)
            ),
